...
def test_guards__get(mocker):
    ...
    mock_get_guards = mocker.patch("src.blueprints.guards.GuardClient.get_guards", return_value=[mock_guard])
...
def test_guards__raises(mocker):
    ...
    assert error.get("cause") == "/guards only supports the GET and POST methods. You specified PUT"
    assert status == 405
...
def test_guard__raises(mocker):
    ...
    assert error.get("status") == 405
    assert error.get("message") == "Method Not Allowed"
    assert error.get("cause") == "/guard/<guard_name> only supports the GET, PUT, and DELETE methods. You specified POST"
    assert status == 405
...
def test_validate__raises_method_not_allowed(mocker):
    ...
    assert error.get("status") == 405
    assert error.get("message") == "Method Not Allowed"
    assert error.get("cause") == "/guards/<guard_name>/validate only supports the POST method. You specified PUT"
    assert status == 405
...
def test_validate__parse(mocker):
    ...
    mock_status = mocker.patch("guardrails.classes.history.call.Call.status", new_callable=PropertyMock)
    mock_status.return_value = "pass"
    mock_guard.history = Stack(
        Call(inputs=CallInputs(prompt="Hello world prompt!"))
    )
    from src.blueprints.guards import validate
    ...
    assert response == {"result": True, "validatedOutput": "Hello world!", "sessionHistory": [{"history": []}], "rawLlmResponse": "Hello world!"}
...
def test_validate__call(mocker):
    ...
    mock_guard = MockGuardStruct()
    mock_tracer = MockTracer()
    mock_request = MockRequest("POST", json={
    ...
    assert response == {"result": False, "validatedOutput": None, "sessionHistory": [{"history": []}], "rawLlmResponse": "Hello world!"}

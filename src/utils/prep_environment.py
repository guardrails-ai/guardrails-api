...
from src.classes import GuardStruct
...
def dynamic_import(package: str):
    ...
    package_path = (
        f"{getcwd()}/{package}"
        if package == "guardrails-custom-validators"
        else package
    )
...
def cleanup_environment(guard: GuardStruct):
    ...
    for p in plugins:
        ...
        logger.dev(f"checking if {p} is in requirements.txt")
        if is_frozen(p) is not True:
            logger.dev(f"uninstalling {p}")
            uninstall(p)
